...
export default function CalendarPage() {
  ...
  <div className="px-4 md:px-8 py-6 max-w-md md:max-w-full mx-auto w-full">
    ...
    {data.nextExam && data.countdown && (
      <div className="rounded-2xl border bg-card p-5 mb-6 md:flex md:items-center md:justify-between md:gap-6">
        <div>
        ...
        <div className="mt-4 md:mt-0 md:text-right shrink-0">
          <div className="flex items-center gap-2 text-primary font-semibold md:justify-end">
            <Clock size={16} />
            {data.countdown.days}d {data.countdown.hours}h {data.countdown.minutes}m
          </div>
          <p className="text-xs text-muted-foreground mt-1">{data.nextExam.duration}</p>
        </div>
      </div>
    )}
      ...
      {data.schedule.map((exam, i) => (
        ...
        >
          <div className="w-12 flex flex-col items-center justify-center shrink-0 text-center border-r border-border pr-3">
            <p className="text-sm font-semibold">{new Date(exam.date).getDate()}</p>
